...
export default registerAs('app', (): AppConfig => {
  ...
  const appName =
    ...
    (process.env.npm_lifecycle_event?.includes('api-server')
      ? 'api-server'
      : process.env.npm_lifecycle_event?.includes('websocket')
        ? 'websocket-service'
        : process.env.npm_lifecycle_event?.includes('admin')
          ? 'admin'
          : 'nestjs-app');
  ...
  if (appName.includes('api-server') || appName === 'api-server') {
    port = parseInt(
      process.env.API_SERVER_PORT || process.env.PORT || '3001',
      10,
    );
  } else if (appName.includes('websocket') || appName.includes('ws')) {
    port = parseInt(
      process.env.WEBSOCKET_PORT || process.env.PORT || '3002',
      10,
    );
