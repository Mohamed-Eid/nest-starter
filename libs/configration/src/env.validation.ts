...
const baseSchema = Joi.object({
  ...
  DATABASE_URI: Joi.string().optional().description('Database connection URI'),
...
export function validateEnvByNodeEnv(env: Record<string, unknown>) {
  ...
  if (nodeEnv === 'production') {
    ...
    if (validated.JWT_REFRESH_SECRET === 'your-refresh-secret-key') {
      warnings.push(
        'JWT_REFRESH_SECRET is using default value - change this in production!',
      );
    }
